...
{str(additional_args)}."""
    ...
    def _execute_step(self, memory_step: ActionStep) -> Generator[StreamEvent]:
            ...
            if self.stream_outputs and hasattr(self.model, "generate_stream"):
                ...
                with Live("", console=self.logger.console, vertical_overflow="visible") as live:
                    for event in output_stream:
                        ...
                        yield ChatMessageStreamEvent("chat_message_stream_delta", event)
            else:
...
>>>>>>> b6a8eeb (Type fixes)
        ...
        if parallel_calls:
            if len(parallel_calls) == 1:
                ...
                yield RunStreamEvent(name="action_output", item=FinalOutput(output=None))
            else:
        ...
        if final_answer_call:
            ...
            yield RunStreamEvent(name="action_call", item=FinalOutput(output=final_answer))
...
class CodeAgent(MultiStepAgent):
    ...
    def _step_stream(self, memory_step: ActionStep) -> Generator[StreamEvent]:
        ...
        yield RunStreamEvent(name="action_output", item=FinalOutput(output=output if is_final_answer else None))
